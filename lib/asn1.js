...
const decodeRange = (module.exports.decodeRange = (buffer, offset, apduLen) => {
  ...
  while (len < apduLen && !decodeIsClosingTagNumber(buffer, offset + len, 5)) {
    ...
    result.push({
      timestamp: new Date(date.value.getFullYear(), date.value.getMonth(), date.value.getDay(), time.value.getHours(), time.value.getMinutes(), time.value.getSeconds()).toISOString(),
      value: value.value,
      status: {
