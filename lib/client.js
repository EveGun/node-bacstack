 ...
 * @param {number=} [options.bacnetDeviceOptions.deviceInstance=10000] - The instance number of this device
 * @param {boolean=} [options.bacnetDeviceOptions.executesReadPropertyMultiple=false] - The instance number of this device
 * @param {boolean=} [options.bacnetDeviceOptions.storeDiscoveredDevices=false] - Should we store discovered devices and display in device_address_binding
 * @param {string=} [options.bacnetDeviceOptions.deviceName='bacnet-node'] - The devicename to be shown when reading device
 * @param {string=} [options.bacnetDeviceOptions.vendorName='Community'] - The vendor name to be shown when reading device
 * @param {number=} [options.bacnetDeviceOptions.vendorIdentifier=9999] - The vendor id to be shown when reading device
 * @param {string=} [options.bacnetDeviceOptions.modelName='bacnet-node'] - The model name to be shown when reading device
 * @param {string=} [options.bacnetDeviceOptions.firmwareRevision='1.00'] - The firmware revision to be shown when reading device
 * @param {string=} [options.bacnetDeviceOptions.applicationSoftwareVersion='1.00'] - The application software version to be shown when reading device
 * @param {string=} [options.bacnetDeviceOptions.location=''] - The location to be shown when reading device
 * @param {string=} [options.bacnetDeviceOptions.description='A bacnet protocol stack written in pure javascript'] - The description to be shown when reading device
 * @example
...
class Client extends EventEmitter {
  ...
  constructor(options) {
    ...
    options = options || {};
    options.bacnetDeviceOptions = options.bacnetDeviceOptions || {};
    ...
    this._deviceAddressBinding = new Map();
    this._settings = {
      ...
      bacnetDeviceOptions: {
        deviceInstance: options.bacnetDeviceOptions.deviceInstance || 10000,
        executesReadPropertyMultiple: options.bacnetDeviceOptions.executesReadPropertyMultiple || false,
        storeDiscoveredDevices: options.bacnetDeviceOptions.storeDiscoveredDevices || false,
        deviceName: options.bacnetDeviceOptions.deviceName || 'bacnet-node',
        vendorName: options.bacnetDeviceOptions.vendorName || 'Community',
        vendorIdentifier: options.bacnetDeviceOptions.vendorIdentifier || 9999,
        modelName: options.bacnetDeviceOptions.modelName || 'bacnet-node',
        firmwareRevision: options.bacnetDeviceOptions.firmwareRevision || '1.00',
        applicationSoftwareVersion: options.bacnetDeviceOptions.applicationSoftwareVersion || '1.00',
        location: options.bacnetDeviceOptions.location || '',
        description: options.bacnetDeviceOptions.description || 'A bacnet protocol stack written in pure Javascript',
      },
    };
  ...
  _performDefaultSegmentHandling(msg, first, moreFollows, buffer, offset, length) {
    ...
    if (!moreFollows) {
      ...
      this._segmentStore = [];
      msg.header.apduType &= ~baEnum.PduConReqBit.SEGMENTED_MESSAGE;
  ...
  _processSegment(msg, server, buffer, offset, length) {
    ...
    if (msg.sequencenumber % msg.proposedWindowNumber === 0 || !moreFollows) {
      const sender = {
        ...
        forwardedFrom: msg.header.sender.forwardedFrom,
        adr: msg.header.sender.adr,
        net: msg.header.sender.adr ? msg.header.sender.net : null,
      };
      this._segmentAckResponse(sender, false, server, msg.invokeId, msg.sequencenumber, msg.proposedWindowNumber);
    }
  ...
  _processServiceRequest(serviceMap, content, buffer, offset, length) {
    ...
    // if message iAm, add device to deviceStore for use in DEVICE_ADDRESS_BINDING
    if (name === 'iAm' && this._settings.bacnetDeviceOptions.storeDiscoveredDevices && content.payload.deviceId !== this._settings.bacnetDeviceOptions.deviceInstance) this._deviceAddressBinding.set(content.payload.deviceId, content.header.sender);
    ...
    } else if (['readProperty', 'readPropertyMultiple'].includes(name) && content.header.expectingReply) {
      /** Hack to respond to readProperty and readPropertyMultiple requests adressed to this device */
      let buffer = this._getBuffer(content.header.sender && content.header.sender.forwardedFrom);
      baNpdu.encode(buffer, baEnum.NpduControlPriority.NORMAL_MESSAGE, content.header.sender.address, null, DEFAULT_HOP_COUNT, baEnum.NetworkLayerMessageType.WHO_IS_ROUTER_TO_NETWORK, 0);
      baApdu.encodeComplexAck(buffer, baEnum.PduType.COMPLEX_ACK, content.service, content.invokeId, content.sequencenumber, content.proposedWindowNumber);

      switch (name) {
        // readProperty
        case 'readProperty':
          if (content.payload.objectId.type === baEnum.ObjectType.DEVICE && content.payload.objectId.instance === this._settings.bacnetDeviceOptions.deviceInstance) {
            try {
              baServices.respondProperty.encodeResponse(buffer, content.payload, this._deviceAddressBinding, this._settings);
              this.sendBvlc(content.header.sender, buffer);
            } catch (e) {
              if (e === 'PROPERTY_IS_NOT_AN_ARRAY') this.errorResponse(content.header.sender, content.service, content.invokeId, baEnum.ErrorClass.PROPERTY, baEnum.ErrorCode.PROPERTY_IS_NOT_AN_ARRAY);
              else if (e === 'INVALID_ARRAY_INDEX') this.errorResponse(content.header.sender, content.service, content.invokeId, baEnum.ErrorClass.PROPERTY, baEnum.ErrorCode.INVALID_ARRAY_INDEX);
              else if (e === 'UNKNOWN_PROPERTY') this.errorResponse(content.header.sender, content.service, content.invokeId, baEnum.ErrorClass.PROPERTY, baEnum.ErrorCode.UNKNOWN_PROPERTY);
              else this.errorResponse(content.header.sender, content.service, content.invokeId, baEnum.ErrorClass.DEVICE, baEnum.ErrorCode.ABORT_BUFFER_OVERFLOW);
            }
          } else {
            this.errorResponse(content.header.sender, content.service, content.invokeId, baEnum.ErrorClass.OBJECT, baEnum.ErrorCode.UNKNOWN_OBJECT);
          }
          break;
        // read propertymultiple
        case 'readPropertyMultiple':
          if (!this._settings.bacnetDeviceOptions.executesReadPropertyMultiple) {
          ...
          try {
            baServices.respondProperty.encodeMultipleResponse(buffer, content.payload, this._settings, this._deviceAddressBinding);
            this.sendBvlc(content.header.sender, buffer);
          } catch (e) {
            this.errorResponse(content.header.sender, content.service, content.invokeId, baEnum.ErrorClass.COMMUNICATION, baEnum.ErrorCode.ABORT_INVALID_APDU_IN_THIS_STATE);
          }
          break;
      }
    } else {
  ...
  _receiveData(buffer, remoteAddress) {
    ...
    let header = {
      ...
      sender: {
        ...
        adr: result.adr || undefined,
        net: result.net || undefined,
        forwardedFrom: null,
      },
    };
    // Check BVLC function
   ...
   * @function bacnet.updateStaticDeviceAddresses
   * @param {number=} address - IP address of the target device.
   * @param {number}= device_id of the device
   *
   */
  updateDeviceAddressBinding(device_id, address) {
    this._deviceAddressBinding.set(device_id, address);
  }
  ...
  whoIs(receiver, options) {
    if (!options) {
      if (receiver && typeof receiver === 'object' && receiver.address === undefined && receiver.forwardedFrom === undefined && (receiver.lowLimit !== undefined || receiver.highLimit !== undefined)) {
  ...
  readProperty(receiver, objectId, propertyId, options, next) {
    next = next || options;
    const settings = {
      maxSegments: options.maxSegments || baEnum.MaxSegmentsAccepted.SEGMENTS_0,
  ...
  writeProperty(receiver, objectId, propertyId, values, options, next) {
    ...
    // if we should write scheduler, do custom encoding
    if (objectId.type === baEnum.ObjectType.SCHEDULE && propertyId === baEnum.PropertyIdentifier.WEEKLY_SCHEDULE) {
      baServices.writeProperty.encodeWeeklySchedule(buffer, objectId.type, objectId.instance, propertyId, settings.arrayIndex, settings.priority, values);
    } else if (objectId.type === baEnum.ObjectType.SCHEDULE && propertyId === baEnum.PropertyIdentifier.EXCEPTION_SCHEDULE) {
      baServices.writeProperty.encodeExceptionSchedule(buffer, objectId.type, objectId.instance, propertyId, settings.arrayIndex, settings.priority, values);
    } else if (objectId.type === baEnum.ObjectType.SCHEDULE && propertyId === baEnum.PropertyIdentifier.EFFECTIVE_PERIOD) {
      baServices.writeProperty.encodeScheduleEffectivePeriod(buffer, objectId.type, objectId.instance, propertyId, settings.arrayIndex, settings.priority, values);
    } else if (objectId.type === baEnum.ObjectType.CALENDAR && propertyId === baEnum.PropertyIdentifier.DATE_LIST) {
      baServices.writeProperty.encodeCalendarDatelist(buffer, objectId.type, objectId.instance, propertyId, settings.arrayIndex, settings.priority, values);
    } else {
  ...
  getEventInformation(receiver, objectId, options, next) {
    ...
    if (objectId) baAsn1.encodeContextObjectId(buffer, 0, objectId.type, objectId.instance);
    this.sendBvlc(receiver, buffer);
    this._addCallback(settings.invokeId, (err, data) => {
      if (err) {
        return void next(err);
