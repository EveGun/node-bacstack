...
class Client extends EventEmitter {
  ...
  _receiveData(buffer, remoteAddress) {
    ...
    switch (result.func) {
      ...
      case baEnum.BvlcResultPurpose.REGISTER_FOREIGN_DEVICE:
        let decodeResult = baServices.registerForeignDevice.decode(
          buffer,
          result.len,
  ...
  readProperty(receiver, objectId, propertyId, options, next) {
    ...
    const settings = {
      ...
      arrayIndex:
        options.arrayIndex || options.arrayIndex === 0 ? options.arrayIndex : baEnum.ASN1_ARRAY_ALL,
    };
    const buffer = this._getBuffer(receiver && receiver.forwardedFrom);
    baNpdu.encode(
      buffer,
      baEnum.NpduControlPriority.NORMAL_MESSAGE | baEnum.NpduControlBit.EXPECTING_REPLY,
      receiver,
      null,
      DEFAULT_HOP_COUNT,
      baEnum.NetworkLayerMessageType.WHO_IS_ROUTER_TO_NETWORK,
  ...
  writeProperty(receiver, objectId, propertyId, values, options, next) {
    ...
    if (objectId.type == 17 && propertyId == baEnum.PropertyIdentifier.WEEKLY_SCHEDULE) {
      baServices.writeProperty.encodeWeeklySchedule(
        buffer,
        objectId.type,
        objectId.instance,
        propertyId,
  ...
  errorResponse(receiver, service, invokeId, errorClass, errorCode) {
    trace(
      `error response on ${JSON.stringify(receiver)} service: ${JSON.stringify(
        service
      )} invokeId: ${invokeId} errorClass: ${errorClass} errorCode: ${errorCode}`
    );
