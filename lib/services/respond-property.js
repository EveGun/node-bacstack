...
module.exports.encodeResponse = (buffer, payload, deviceAddressBinding, options) => {
  ...
  if (property === 'OBJECT_LIST') encodeDeviceObjectList(buffer, payload.objectId.instance);
  else if (property === 'STRUCTURED_OBJECT_LIST') encodeEmptyList(buffer);
  else if (property === 'PROPERTY_LIST') encodeDevicePropertyList(buffer, options);
  else if (property === 'DEVICE_ADDRESS_BINDING') encodeDeviceAddressBinding(buffer, deviceAddressBinding);
  else {
    const prop = getDeviceProperties(options)[property];
    if (!prop) throw `UNKNOWN_PROPERTY: ${property}`;
...
module.exports.encodeMultipleResponse = (buffer, payload, options, deviceAddressBinding) => {
  ...
  objectMap.forEach((data, obj) => {
    ...
    if (obj.type !== 8) {
      ...
      if (data.length === 1 && data[0].id === baEnum.ObjectType.DEVICE) var points = availableData;
      else {
        var points = [];
        data.forEach((row) => {
      ...
      points.forEach((point) => {
        ...
        if (obj.instance !== options.bacnetDeviceOptions.deviceInstance || obj.type !== baEnum.ObjectType.DEVICE) {
          ...
          switch (point.id) {
            case 76: //object list
              encodeDeviceObjectList(buffer, options.bacnetDeviceOptions.deviceInstance, 4);
              break;
            case 371:
              encodeDevicePropertyList(buffer, 4, options);
              break;
            case 30: // Device address binding
              //encodeEmptyList(buffer, 4);
              encodeDeviceAddressBinding(buffer, deviceAddressBinding, 4);
              break;
            case 255:
              //encode error
              baAsn1.encodeOpeningTag(buffer, 5);
...
const encodeDeviceAddressBinding = (buffer, devices, tagNumber = 3) => {
  baAsn1.encodeOpeningTag(buffer, tagNumber);
...
const encodeDeviceObjectList = (buffer, objectInstance, tagNumber = 3) => {
  baAsn1.encodeOpeningTag(buffer, tagNumber);
  baAsn1.encodeApplicationObjectId(buffer, baEnum.ObjectType.DEVICE, objectInstance);
  baAsn1.encodeClosingTag(buffer, tagNumber);
};

const encodeDevicePropertyList = (buffer, tagNumber = 3, options) => {
  baAsn1.encodeOpeningTag(buffer, tagNumber);
  for (const prop of Object.values(getDeviceProperties(options))) {
...
const getDeviceProperties = (options = {}) => {
  return {
    ...
    MAX_APDU_LENGTH_ACCEPTED: {
      id: 62,
      datatype: 2, //Unsigned integer
      value: baEnum.MaxApduLengthAccepted.OCTETS_1476, // Max UDP payload
    },
