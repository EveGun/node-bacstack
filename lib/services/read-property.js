...
module.exports.decodeAcknowledge = (buffer, offset, apduLen) => {
  ...
  len++;
  // decode scheduler
  if (objectId.type === 17 && property.id === baEnum.PropertyIdentifier.WEEKLY_SCHEDULE) {
    let res = baAsn1.decodeWeeklySchedule(buffer, offset + len, apduLen, objectId);
    if (!res) return undefined;
    values.push(...res.values);
    len = res.len;
  } else if (objectId.type === 17 && property.id === baEnum.PropertyIdentifier.EXCEPTION_SCHEDULE) {
    let res = baAsn1.decodeExceptionSchedule(buffer, offset + len, apduLen, objectId);
    if (!res) return undefined;
    values.push(...res.values);
    len = res.len;
  } else if (objectId.type === 6 && property.id === baEnum.PropertyIdentifier.DATE_LIST) {
